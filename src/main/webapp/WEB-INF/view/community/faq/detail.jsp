...
<div class="user-content" style="max-width:860px;">
    ...
    <c:if test="${sessionScope.loginUser.user_role == 2}">
        <div style="display:flex;justify-content:flex-end;gap:8px;margin-bottom:24px;">
            <a href="${pageContext.request.contextPath}/community/board/form?boardid=2&board_no=${board.board_no}"
               class="btn btn-black btn-sm">수정</a>
            <form action="${pageContext.request.contextPath}/community/board/delete"
                  method="post" style="display:inline;">
                <input type="hidden" name="boardid" value="2">
                <input type="hidden" name="board_no" value="${board.board_no}">
                <button type="button" class="btn btn-ghost btn-sm" onclick="doDelete(this)">삭제</button>
            </form>
        </div>
    </c:if>
    ...
    <div class="post-nav">
        <c:if test="${not empty prevPost}">
            <div class="post-nav-item">
                ...
                <a href="${pageContext.request.contextPath}/community/board/detail?boardid=2&board_no=${prevPost.board_no}">
        ...
        <c:if test="${not empty nextPost}">
            <div class="post-nav-item">
                ...
                <a href="${pageContext.request.contextPath}/community/board/detail?boardid=2&board_no=${nextPost.board_no}">
