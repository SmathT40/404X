    ...
    <form id="inquiryForm"
        ...
        <div class="form-row">
            ...
            <div class="file-upload-row" style="flex:1;display:flex;gap:10px;">
                <input type="text" id="inqFileName" class="form-control"
                       placeholder="파일을 첨부해주세요." readonly value="${board.fileurl}">
                <button type="button" class="btn btn-black btn-sm"
                        onclick="$('#inqFile').click()">파일첨부</button>
                <input type="file" id="inqFile" name="board_file_path" style="display:none;"
                       onchange="$('#inqFileName').val(this.files[0].name)">
            </div>
        </div>
